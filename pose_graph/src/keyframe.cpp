...
bool KeyFrame::findConnection(KeyFrame* old_kf, int& match_num_search, int& match_num_pnp)
{
	TicToc tmp_t;
	...
	*/
	...
	double relative_yaw;
	match_num_search = (int)matched_2d_cur.size();
	match_num_pnp = 0;
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_3d, status);
	    reduceVector(matched_id, status);
		match_num_pnp = matched_2d_cur.size();
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 1.0)
	    {
