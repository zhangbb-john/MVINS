...
void gt_callback(const nav_msgs::Odometry odom)
{
    ...
    {    
        ...
        baseTgt = est_position - baseRgt * Vector3d(gt_position.x(), gt_position.y(), gt_position.z())  ;
        return;
    }
...
struct SmoothnessResidual {
    ...
    bool operator()(const T* const xy1, const T* const xy2, const T* const xy3, T* residual) const {
        ...
        residual[0] = T(10.0) * (xy3[0] - T(2.0) * xy2[0] + xy1[0]);
        residual[1] = T(10.0) * (xy3[1] - T(2.0) * xy2[1] + xy1[1]);
        return true;
    }
};
void path_callback(const nav_msgs::Path& path)
{
    ...
    if (path.poses.empty() || count_path % 5 > 0) 
    {
    ...
    size_t push_id = path.poses.size() - 50;
    if (push_id < 0)
    {
        push_id = 0;
    }
